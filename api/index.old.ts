...
app.get('/api/healthchecker', withDebugLogs, async (c) => {
  ...
  const boundaryIntervalSeconds = boundaryIntervalMs / 1000
  ...
  while (elapsed < totalDurationMs) {
    ...
    const currentInterval = Math.floor(elapsed / boundaryIntervalMs)
    const lastInterval = Math.floor(lastCheckAt / boundaryIntervalMs)
    
    if (currentInterval > lastInterval && elapsed < totalDurationMs) {
